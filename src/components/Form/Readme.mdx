...
Any other props provided to `SelectButton` will be passed down to `ButtonGroup`.
So we can pass down `flexDirection='column'` for example or positioning props
like `marginBottom="base"`.
...
<Playground>
  {() => {
    ...
    return (
      <Flex>
        <Box margin="base" padding="base" maxWidth="350px" bg="whiteout.dark">
          <SelectButton
            name="durations"
            title="Select the duration of your experience"
            options={[
...
## SingleSelectButton with Other

We can use the `SingleSelectButton` with an `enableOther` flag.
